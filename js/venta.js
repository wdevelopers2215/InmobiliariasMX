...
function eliminarCookies() {
  cookies.remove(urlcookies, "itemspagV", function() {});
  cookies.remove(urlcookies, "filtroV", function() {});
  cookies.remove(urlcookies, "imagenesV", function() {});
  cookies.remove(urlcookies, "itemsV", function() {});
  cookies.remove(urlcookies, "rangeV", function() {});
  cookies.remove(urlcookies, "rbV", function() {});
  cookies.remove(urlcookies, "datoV", function() {});
  cookies.remove(urlcookies, "actpageV", function() {});
}
...
(function() {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    buscarCookie("itemspagV", [])
      .then(val => {
        return buscarCookie("filtroV", val);
      })
      .then(val => {
        return buscarCookie("imagenesV", val);
      })
      .then(val => {
        return buscarCookie("itemsV", val);
      })
      .then(val => {
        return buscarCookie("rangeV", val);
      })
      .then(val => {
        return buscarCookie("rbV", val);
      })
      .then(val => {
        return buscarCookie("datoV", val);
      })
      .then(val => {
        return buscarCookie("actpageV", val);
      })
      .then(val => {
        // Promise.all([
      ...
      .on("child_added", function(snapshot) {
        ...
        obtenerUrlCasa(casa).then(function(image) {
          ...
          obtenerUrlProfile(casa).then(function(profile) {
            ...
            // }
            buscarCookie("itemspagV", []).then(val => {
              if (contFilasMI < 15) {
    ...
    function crearItem(casa, img, perfil) {
      ...
      <div class="contenedor-inmueble clearfix">
        ...
        <img src="${perfil}" onclick="mostrarContacto(${casa.correoUsuario},${
        casa.numeroUsuario
      })">
        </span>
        <div class="nombre-descripcion">
    ...
    function filtrarElementos(e) {
      ...
      //   value: "dummy"
      // };
      setCookie("itemspagV", itemsPaginado);
      // cookies.set(
      ...
      setCookie("actpageV", JSON.stringify(paginaActual));
      // cookies.set(
      //   {
      //     url: urlcookies,
      //     name: "actpage",
      ...
      setCookie("datoV", document.getElementById("IF").value);
      // cookies.set(
      //   {
      //     url: urlcookies,
      //     name: "dato",
    ...
    function buscarItem(dato, filtro, array) {
      // localStorage.setItem("imagenes", JSON.stringify(arregloimg));
      setCookie("imagenesV", JSON.stringify(arregloimg));
      // cookies.set(
      //   {
      //     url: urlcookies,
      //     name: "imagenes",
      ...
      switch (filtro) {
        case "Todo":
          ...
          // if (subcat != "all") {
          matrizFiltro = array;
          setCookie("itemsV", JSON.stringify(array));
          // }
